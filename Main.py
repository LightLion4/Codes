    ...
    def forward(self, bVec, fwd_noise, fb_noise, isTraining, eachbatch):
        ...
        for idx in range(self.args.K + 1):
            forward_noise_input = torch.cat([combined_noise[:, :idx, 1:],
                                             torch.zeros(self.args.batchSize, 1, self.args.rate - 1 + self.common).to(
                                                 self.args.device)], dim=1)
            src1 = torch.cat(
                [bVec_add0[:, :(idx + 1), :], combined_noise[:, :(idx + 1), 0].unsqueeze(2), forward_noise_input],
                dim=2)
            if args.mask == 1:
