...
for i = 1:length(pe_values)
    ...
    % 利用fzero来寻找SNR1的值
    SNR1_initial_guess = 5; % 更合理的初始猜测值可能需要根据情况调整
    try
...
% ldpc_db_values1 = [2.78,2.91, 3.15, 3.31,3.62, 3.9, 4.3, 4.95 5.3];
% ldpc_pe_values1 = [10^-1, 10^-2, 10^-3, 10^-4, 10^-5,10^-6,10^-7,10^-8,10^-9];

ldpc_db_values1 = [2.46,2.61, 2.81, 3.11,3.42, 3.81,4.45];
ldpc_pe_values1 = [10^-1, 10^-2, 10^-3, 10^-4, 10^-5,10^-6,10^-7];
...
% 画SK编码的图（有噪）
% semilogy(SNR1_dB_values, pe_values, '-s', 'DisplayName', 'Modulo-Lattice Code, N=40', 'LineWidth', 1.5);
% hold on; % 保持当前图形，以便在相同的坐标轴上绘制另一条线

% 画LDPC编码的图semilogy(ldpc_db_values, ldpc_pe_values, '-*', 'DisplayName', 'LDPC Code(N=2400，4-PAM)', 'LineWidth', 1.5);

% 画LDPC编码的图
semilogy(ldpc_db_values1, ldpc_pe_values1, '-*', 'DisplayName', 'Attention Code(K=20, N=40)', 'LineWidth', 1.5);
...
% title('', 'FontWeight', 'bold','FontName','Times New Roman');
title('$\delta_F = 0.5$', 'Interpreter', 'latex','FontWeight', 'bold','FontName','Times New Roman');
...
% detaf=0.5;
% an=(1-detaf)*(N-1);
%     L =1/3*(qfuncinv(pe/4/an))^(2);
%     fa1=1+L*SNR1/SNR2;
%     fa2=1/(1-L/SNR2);
%     R =1/2/N*log2(3*SNR1/(qfuncinv(pe/4))^(2)*(1+SNR1/fa1/fa2)^(an));
% end
...
detaf=0.2;
an=(1-detaf)*(N-1);
R =an*log2(1+SNR1)/2/N-log2(sqrt(1/12/SNR1)*2*qfuncinv(pe/2))/N;
end
